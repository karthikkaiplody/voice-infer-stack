"""Render one turn as a self-contained HTML page you can open in a browser.
...
<p class="sub">Every bar sits where that stage actually ran. The red line is
800&nbsp;ms, roughly what human turn-taking costs. Bars that start before the
one above finishes are running at the same time.</p>
{"".join(out)}
<p class="legend">Generated by <code>viewer.py</code> from OpenTelemetry spans.
Nothing here is drawn by hand; re-run <code>make bench</code> and regenerate to
