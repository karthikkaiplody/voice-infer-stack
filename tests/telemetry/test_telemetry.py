...
from voice_agent.telemetry.tracing import JsonlSpanExporter, LiveSpanProcessor, TurnSpanObserver
...
def test_repository_schema_matches_runtime_contract():
    ...
    assert set(schema["properties"]["schema_version"]["enum"]) == SUPPORTED_SCHEMA_VERSIONS
...
def test_live_workloads_are_unique_and_noncomparable():
    from voice_agent.pipeline.agent import workload_identity
...
def test_local_control_rejects_bad_host_and_origin():
    from voice_agent.server.live import local_control_only
...
def test_browser_errors_use_fixed_controlled_classifications(error, expected):
    from voice_agent.server.live import browser_error_classification
