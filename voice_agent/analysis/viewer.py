...
models and no install, and is what someone reads first after cloning.
    ...
    python3 -m voice_agent.analysis.viewer                        # committed reference traces
    python3 -m voice_agent.analysis.viewer --traces mine.jsonl
    python3 -m voice_agent.analysis.viewer --open
...
def collect(spans, win):
    ...
    for name in budget.STAGES:
        ...
        stages.append({
            ...
            "ms": measurement.duration_sum_ms(ivs),
...
def main():
    ...
    ap.add_argument("--out", default=str(paths.ARTIFACTS_DIR / "viewer.html"))
