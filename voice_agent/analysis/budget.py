...
names in SPANS.md.
    ...
    uv run python -m voice_agent.analysis.budget                                   # the committed turn
    uv run python -m voice_agent.analysis.budget --traces artifacts/live-traces.jsonl   # yours
...
def summarize(spans, win):
    ...
    covered_ms = measurement.union_ms(compute_ivs)
    sum_ms = measurement.duration_sum_ms(compute_ivs)
    ...
    rows = [{"stage": n,
             "duration_ms": measurement.duration_sum_ms(inside[n]),
             "count": raw_counts[n],
    ...
    for i, a in enumerate(compute):
        for b in compute[i + 1:]:
            ov = measurement.intersection_ms(inside[a], inside[b])
            if ov > 1:
                pairs.append((a, b, ov))
...
def main():
    ...
    ap.add_argument("--traces", default=str(paths.ARTIFACTS_DIR / "reference-traces.jsonl"),
