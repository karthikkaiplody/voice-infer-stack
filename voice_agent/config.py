...
class Config:
    # --- models ---
    # --- which engine runs each stage (see pipeline/factory.py) ---
    ...
    # like a broken pipeline: the page sits at "Listening" forever. The page warns
    # when the input is exactly silent, but not when it is heard and too quiet.
    ...
    # --- agent ---
    # Which agent under `agents/` to run. Empty means the original behaviour:
    # the inline `system_prompt` above, no retrieval. That is what the benchmark
    # and `make trace` use, so their reference numbers stay comparable. `make
    # live` selects the library agent, whose facts live in a markdown file that
    # is searched on every turn instead of sitting in the prompt.
    agent: str = ""
    # How many passages of the agent's knowledge are put in front of the model.
    # More gives it more to work from and a longer prompt to read.
...
def _from_env() -> Config:
    ...
    for f in fields(Config):
        ...
        elif str(f.type).startswith("int"):
            # An optional int, like `audio_device: int | None = None`. Its default is
            # None, so the check above cannot see that it is a number. Empty means None.
            changes[f.name] = int(env) if env.strip() else None
